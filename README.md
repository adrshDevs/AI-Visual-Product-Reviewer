...
AI-powered visual product reviewer. Upload a photo of a product (or just type its name), and IntelliBuy identifies it, researches it, and gives you a full buying brief — pros/cons, price comparisons across major retailers, a 12-month price history, review-authenticity scoring, and smarter alternatives — all generated by a single call to Google Gemini.
...
Production app — a React (Vite) frontend talking to a FastAPI backend over a REST endpoint. This is the primary, actively developed app and the one run.sh launches.
Standalone prototype — a single-file Streamlit app (app.py) that does the same analysis end-to-end in one process, useful for quick local demos without building the frontend.
...
Search screen — upload a product photo, ask a specific question, or just search by name.

![IntelliBuy search screen](assets/hero.png)
AI insight & engineered details — Gemini's identification, build breakdown, capabilities, strengths, and limitations.

![AI insight and engineered details](assets/ai-insight.png)
Price trends — a 12-month price history chart with hover detail.

![Price trend chart](assets/price-trends.png)
Review authenticity — a genuine/fake breakdown of sample reviews with reasoning.

![Integrity assessment and verified reviews](assets/integrity-assessment.png)
Retailer comparison & alternatives — trusted stores with trust scores, plus competing products.

![Trusted platforms and alternate options](assets/trusted-platforms.png)
Features
...
The user uploads an image and/or types a prompt in the React UI.
The frontend sends a multipart/form-data request to POST /api/analyze.
The backend (backend/main.py) downsizes and compresses any uploaded image with Pillow, then sends it — along with a detailed JSON-schema instruction prompt — to Gemini (gemini-3.5-flash) in JSON response mode.
Gemini returns one JSON object containing the product identification, pros/cons, sample reviews, per-review authenticity verdicts, retailer prices, and alternative products.
The backend enriches that JSON: it builds real search URLs per retailer, fills in a synthesized 12-month price history if Gemini's is too short, and (for text-only searches) attaches a placeholder product image.
The enriched JSON is returned to the frontend, which renders the price chart (Recharts), the review-authenticity gauge, retailer cards, and alternative-product cards.
Follow-up questions about the same product reuse the existing image and simply send a new prompt, letting Gemini decide (is_new_product) whether it's a refinement or a brand-new search.
